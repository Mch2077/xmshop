...
<body>
	<div id="brall">
		...
		<div id="condition" style="text-align: center">
			<form action="${pageContext.request.contextPath}/getprobypage" id="myform" method="get">
		...
		<div id="table">
			<div id="top">
				<input type="button" class="btn btn-warning" id="btn1" value="新增商品" onclick="addproduct()">
			</div>
			<!--显示分页后的商品-->
			<div id="middle">
				...
				<div class="footNum">
					<ul>
						<c:choose>
							...
							<c:otherwise>
								<li class="pre"><a
									href="${pageContext.request.contextPath}/getprobypage?page=${pb.page-1}&name=${name}&typeid=${typeid}">
										上一页</a></li>
							</c:otherwise>
						</c:choose>
						<c:forEach begin="1" end="${pb.pages}" step="1" var="index">
							<c:choose>
								...
								<c:otherwise>
									<li class="num"><a
										href="${pageContext.request.contextPath}/getprobypage?page=${index}&name=${name}&typeid=${typeid}">${index}</a></li>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						<c:choose>
...
<script type="text/javascript">
    ...
    //修改函数
    function modify(id) {
        location.href = "${pageContext.request.contextPath}/toupdateproductpage?id="+id;//get
    }
</script>
</html>